...
export function sourceFingerprint(root='.') {
  const files=[];
  function walk(relative){const location=path.join(root,relative);if(!existsSync(location))return;for(const e of readdirSync(location,{withFileTypes:true})){const name=path.posix.join(relative,e.name);if(e.isDirectory())walk(name);else if(!/metadata-snapshot|\.pending\./.test(name))files.push(name);}}
  for(const dir of ['src','lib','tools','tests','data'])walk(dir);
  for(const file of ['build.mjs','package.json','package-lock.json'])if(existsSync(path.join(root,file)))files.push(file);
