...
function retrievalDate(value,now){
  ...
  return Boolean(match&&calendarDay(match[1])&&Number.isFinite(Date.parse(value))&&Date.parse(value)<=now+300000);
}
function names(value){return Array.isArray(value)&&value.every(v=>typeof v==='string'&&v.trim());}
export function validateSnapshot(snapshot,now=Date.now()) {
  ...
  for(const [slug,d] of Object.entries(snapshot)){
    ...
    for(const field of ['overview','network'])if(d[field]!=null&&typeof d[field]!=='string')fail(slug+': invalid '+field);
    for(const field of ['genres','providers'])if(d[field]!=null&&!names(d[field]))fail(slug+': invalid '+field);
    if(d.tmdbRating!=null&&(!Number.isFinite(d.tmdbRating)||d.tmdbRating<0||d.tmdbRating>10))fail(slug+': invalid TMDB rating');
    if(d.availabilityStatus!=null&&!['checked','stale','unconfirmed'].includes(d.availabilityStatus))fail(slug+': invalid availability status');
    if(d.availabilityCheckedAt!=null&&!retrievalDate(d.availabilityCheckedAt,now))fail(slug+': invalid/future retrieval date');
    // Scheduled season dates may be in the future; impossible calendar dates may not.
    if(d.seasonAired!=null&&!calendarDay(d.seasonAired))fail(slug+': invalid season air date');
    ...
    for(const [code,list] of Object.entries(d.providersByRegion||{}))if(!/^[A-Z]{2}$/.test(code)||!names(list))fail(slug+': invalid provider list');
